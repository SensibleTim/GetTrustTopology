#************************************************ 
#GetTrustTopology.ps1 
# Version 1.0 
# Date: 3/4/2015 
# Author: Tim Springston
# Description:  This script queries for the user and computer domain
#  and returns details about the trusts which they have. The trusts can
#  be of forest, internal, or external type. The script gives config
#  info for the trusts.
#************************************************ 
cls
Trap [Exception]
...
$TrustsOutput = "C:\windows\temp\TrustTopology.txt"
...
if ($Trusts.count -eq 1)
	{	
				$Targetname = $Trusts.TargetName.ToString()
			...
			$TrustTitleString = 'Trust Details for ' + ($Trusts.SourceName.ToString()) + '|' + ($Trusts.TargetName.ToString())
		    add-member -inputobject $DomainTrustObject -membertype noteproperty -name "Trust Name" -value $TrustTitleString
		    add-member -inputobject $DomainTrustObject -membertype noteproperty -name "Local Domain (Source)" -value $Trusts.SourceName
		    add-member -inputobject $DomainTrustObject -membertype noteproperty -name "Trusted Domain (Target)" -value $Trusts.TargetName
		    add-member -inputobject $DomainTrustObject -membertype noteproperty -name "Trust Direction" -value $Trusts.TrustDirection
		    add-member -inputobject $DomainTrustObject -membertype noteproperty -name "Trust Type" -value $Trusts.TrustType
			if ($Trusts.TrustType -eq 'Forest')
		    	{
				if ($Trusts.TrustedDomainInformation -ne $null)
					{
					foreach ($TDI in $Trusts.TrustedDomainInformation)
						{
		...
		foreach	($Trust in $Trusts)
			...
			add-member -inputobject $DomainTrustObject -membertype noteproperty -name "Selective Authentication" -value $SelectiveAuthStatus
			$DomainTrustObject | FL * | Out-File $TrustsOutput  -Append -encoding UTF8
			$DomainTrustObject | FL * | Write-Host
			#Set values to $null for next trust check.
			$SidFilteringStatus = $null
			$InterForestTrustObject = $null
			}
...
Write-Host "Trust topology information collected. Results also available in $TrustsOutput"
